...
from .models import (GraphicsCards, Motherboards, PowerSupplyUnits, Processors,
                     Ram, Reviews, Storage)
...
class ComponentService:
    ...
    def _get_components_by_category(
        cls, category: str, brand: str = None
    ) -> List[Dict[str, Any]]:
        ...
        for component in queryset:
            components.append(
                cls._build_component_dict(component, category, component_type)
            )
    ...
    def _build_component_dict(
        cls, component: Any, category: str, component_type: str
    ) -> Dict[str, Any]:
    ...
    def _filter_by_price_range(
        cls, components: List[Dict[str, Any]], price_range: str
    ) -> List[Dict[str, Any]]:
        ...
        return [
            c for c in components if c["price"] and min_price < c["price"] <= max_price
        ]

    @classmethod
    def _sort_components(
        cls, components: List[Dict[str, Any]], sort_by: str
    ) -> List[Dict[str, Any]]:
    ...
    def get_all_manufacturers(cls) -> List[str]:
        ...
        try:
            for model in cls.COMPONENT_MODELS.values():
                model_manufacturers = model.objects.values_list(
    ...
    @classmethod
    def get_component_by_type_and_id(
        cls, component_type: str, component_id: int
    ) -> Tuple[Any, str]:
        """
        ...
        try:
            ...
            raise ValueError(
                f"Component not found: {component_type} with id {component_id}"
            )
    ...
    def _get_processor_specs(component) -> Dict[str, Any]:
        ...
        return {
            ...
            "Socket": (
                str(getattr(component, "socket", "N/A"))
                if getattr(component, "socket", None)
                else "N/A"
            ),
            "Počet jader": getattr(component, "corecount", "N/A"),
            "Frekvence": (
                f"{component.clock} MHz" if getattr(component, "clock", None) else "N/A"
            ),
    ...
    def _get_ram_specs(component) -> Dict[str, Any]:
        ...
        return {
            "Výrobce": getattr(component, "manufacturer", "N/A"),
            "Typ": (
                str(getattr(component, "type", "N/A"))
    ...
    def _get_motherboard_specs(component) -> Dict[str, Any]:
        ...
        return {
            ...
            "Formát": (
                str(getattr(component, "format", "N/A"))
                if getattr(component, "format", None)
                else "N/A"
            ),
    ...
    def get_similar_components(
        cls, component: Any, component_type: str, limit: int = 4
    ) -> QuerySet:
        ...
        try:
            ...
            model = cls.COMPONENT_MODELS[category]
            return model.objects.filter(manufacturer=component.manufacturer).exclude(
                id=component.id
            )[:limit]
        except Exception:
            return []
...
class ReviewService:
    ...
    def get_component_reviews(
        cls, component: Any, component_type: str, limit: int = 10
    ) -> QuerySet:
    ...
    def get_review_statistics(
        cls, component: Any, component_type: str
    ) -> Dict[str, Any]:
...
class SearchService:
    ...
    def _search_in_components(
        cls, query: str, selected_category: str = None
    ) -> List[Dict[str, Any]]:
        ...
        if selected_category and selected_category in category_mapping:
            ...
            selected_category and selected_category in ComponentService.COMPONENT_MODELS
        ):
                    ...
                    results.append(
                        {
                            ...
                            "description": ComponentService._get_component_description(
                                component, category
                            ),
                            "url": f"/components/{component_type}/{component.id}/",
                            "price": (
                                float(component.price) if component.price else None
                            ),
                            "rating": component.rating,
                            ...
                            "relevance": cls._calculate_relevance(
                                query, component.name, component.manufacturer
                            ),
                        }
                    )
            except Exception:
                ...
                results.append(
                    {
                        ...
                        "relevance": cls._calculate_relevance(
                            query, review.title, review.summary or ""
                        ),
                    }
                )
        except Exception:
    ...
    def _sort_search_results(
        cls, results: List[Dict[str, Any]], sort: str
    ) -> List[Dict[str, Any]]:
...
class BreadcrumbService:
    ...
    @classmethod
    def get_component_breadcrumbs(
        cls, component_type: str, component: Any
    ) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {
                "name": ComponentService.TYPE_DISPLAY_NAMES.get(
