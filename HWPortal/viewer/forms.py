...
class ReviewForm(forms.ModelForm):
        ...
        widgets = {
            ...
            "rating": forms.Select(
                choices=[
                    (
                        i,
                        f'{i} hvězdič{"ky" if i in [2, 3, 4] else "ka" if i == 1 else "ek"}',
                    )
                    for i in range(1, 6)
                ],
                attrs={
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.component_type_param:
            ...
            if self.component_id_param:
                self.fields["component_choice"].initial = (
                    f"{self.component_type_param}_{self.component_id_param}"
                )
        else:
            self.fields["component_choice"].choices = [
                ("", "Nejdříve vyberte typ komponenty...")
            ]
    ...
    def get_components_by_type(self, component_type):
        ...
        if component_type == "processor":
            for proc in Processors.objects.all().order_by("manufacturer", "name"):
                components.append(
                    (f"processor_{proc.id}", f"{proc.manufacturer} {proc.name}")
                )

        elif component_type == "graphics_card":
            for gpu in GraphicsCards.objects.all().order_by("manufacturer", "name"):
                components.append(
                    (f"graphics_card_{gpu.id}", f"{gpu.manufacturer} {gpu.name}")
                )
